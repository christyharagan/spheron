      ...
      try {
        ...
        if (packet) {
          ...
          if (packet instanceof Buffer) {
            ...
            switch (packet.SOP2) {
              case 0xFF:
                anEmitter.emit('response', packet);
                break;
              case 0xFE:
                switch (packet.ID_CODE) {
                  case 0x1:
                    anEmitter.emit('power_notifications', packet.DATA /*TODO*/);
                    return;
                  case 0x2:
                    anEmitter.emit('level_1_diagnostic_response', packet.DATA /*TODO*/);
                    return;
                  case 0x3:
                    anEmitter.emit('sensor_data_streaming', packet.DATA /*TODO*/);
                    return;
                  case 0x4:
                    anEmitter.emit('config_block_contents', packet.DATA /*TODO*/);
                    return;
                  case 0x5:
                    anEmitter.emit('pre_sleep_warning');
                    return;
                  case 0x6:
                    anEmitter.emit('macro_markers', packet.DATA /*TODO*/);
                    return;
                  case 0x7:
                    anEmitter.emit('collision_detected', packet.DATA /*TODO*/);
                    return;
                  case 0x8:
                    anEmitter.emit('orbbasic_print_message', packet.DATA.toString());
                    return;
                  case 0x9:
                    anEmitter.emit('orbbasic_error_message', packet.DATA.toString());
                    return;
                  case 0xA:
                    anEmitter.emit('orbbasic_error_message_binary', packet.DATA /*TODO*/);
                    return;
                  case 0xB:
                    anEmitter.emit('self_level_result', packet.DATA /*TODO*/);
                    return;
                  case 0xC:
                    anEmitter.emit('gyro_axis_limit_exceeded', packet.DATA /*TODO*/);
                    return;
                }

                anEmitter.emit('message', packet);
                break;
            }
          }
        }
